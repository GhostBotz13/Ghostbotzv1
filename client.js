...
const connect = async () => {
   ...
   client.ws.on('CB:call', async json => {
      ...
      if (json.content[0].tag == 'penawaran') {

         biarkan objek = json.content[0].attrs['call-creator']

         menunggu Func.delay(2000)

         menunggu client.updateBlockStatus(objek, 'blok')
   ...
   // Restart otomatis jika penggunaan ram sudah mencapai batas, jika ingin menggunakan masukkan ukuran ram dalam byte

   const ramCheck = setInterval(() => {

      var ramUsage = process.memoryUsage().rss

      jika (ramUsage >= 900000000) { // 900 MB
   ...
   }, 60 * 1000) // Memeriksa setiap 1 menit
   ...
   setInterval(async () => {
      ...
      jika (tmpFiles.length > 0) tmpFiles.map(v => fs.unlinkSync('./temp/' + v))

   }, 60 * 1000 * 5)

   setInterval(async () => {

      jika (global.db) menunggu props.save(global.db)
